...
const apiFetch = async (endpoint: string, options: RequestInit = {}) => {
    ...
    // FIX: Use Headers object for robust and type-safe header management.
...
export const updateSiteSettings = (settings: SiteSettings): Promise<void> => apiFetch('/admin/data/settings', { method: 'PUT', body: JSON.stringify(settings) });
export const updateManagedProducts = (products: Product[]): Promise<void> => apiFetch('/admin/data/products', { method: 'PUT', body: JSON.stringify(products) });
export const updateProductSizes = (sizes: Record<ProductType, ProductSize[]>): Promise<void> => apiFetch('/admin/data/sizes', { method: 'PUT', body: JSON.stringify(sizes) });
